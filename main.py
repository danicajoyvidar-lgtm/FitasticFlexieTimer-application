...
class FITasticFlexieTimer:
    ...
    def __init__(self, root):
        ...
        # LOCK SYSTEM (FEATURE 1 REQUIRED)
    ...
    def validate_user_info(self):
        try:
            ...
            weight = self.user_weight.get().strip()

            if not name or not weight:
                messagebox.showwarning(
                    "Missing Information",
                    "Please enter Name and Weight first before using the app."
                )
                return False
    ...
    def schedule_save(self, event=None):
        try:
            ...
            self.save_after_id = self.root.after(800, self.save_data_silent)
        except:
            pass
    ...
    def load_data(self):
        try:
            if os.path.exists(self.DATA_FILE):
                ...
                self.completed_exercises = data.get("completed_exercises", 0)
    ...
    def create_ui(self):
        ...
        tk.Label(user_card, text="👤 User Information",
        ...
        # AVAILABILITY
        availability_card = tk.Frame(left_panel, bg=self.light)
        availability_card.pack(fill="x", pady=15)
        ...
        for day in days:
            frame = tk.Frame(availability_card, bg=self.light)
            frame.pack(anchor="w", padx=20, pady=3)

            tk.Label(frame, text=day, width=12, bg=self.light).pack(side="left")

            entry = tk.Entry(frame, width=20)
            entry.pack(side="left", padx=10)

            if hasattr(self, "saved_availability") and day in self.saved_availability:
                entry.insert(0, self.saved_availability[day])

            entry.bind("<KeyRelease>", self.schedule_save)
            self.availability[day] = entry
        ...
        # TIMER
        right_card = tk.Frame(right_panel, bg=self.light, padx=20, pady=20)
        ...
        self.progress_label = tk.Label(
            progress_card,
            text=f"{self.completed_exercises} / {self.total_exercises}",
            bg=self.light
        )
    ...
    def display_exercises(self):
        ...
        for day, exercises in self.weekly_plan.items():
            tk.Label(self.exercise_frame, text=day,
                     font=("Arial", 12, "bold"),
                     bg=self.light, fg=self.primary).pack(anchor="w")
    ...
    def finish_exercise(self):
        try:
            self.current_exercise.mark_completed()
        except:
            pass
        ...
        percent = (self.completed_exercises / self.total_exercises) * 100
